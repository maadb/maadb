...
- **0.12.0 (next minor)** — folds in HTTP `/mcp` Origin validation (MCP Streamable HTTP conformance, DNS-rebinding defense; merged unreleased) and generated agent-instruction correctness fixes (merged unreleased; release note: reindex refreshes MAAD.md content in existing projects) alongside the concurrency/durability work.
