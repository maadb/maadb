...
- **0.12.0 (next minor)** — folds in HTTP `/mcp` Origin validation (MCP Streamable HTTP conformance, DNS-rebinding defense; merged unreleased), generated agent-instruction correctness fixes (merged unreleased), and the managed-instruction lifecycle (merged unreleased: stamped MAAD.md/_skills files, `maad instructions check/refresh` + `maad_instructions` MCP tool, static MAAD.md — **behavior change: reindex no longer rewrites MAAD.md**; boot emits an `instructions_outdated` advisory only) alongside the concurrency/durability work.
