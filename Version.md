...
## 0.7.7 — 2026-05-01
Schema-cache coherence across concurrent writers.
...
## 0.7.6 — 2026-05-01
Parser / write-path security hardening.
...
## 0.7.4 — 2026-05-01
Reindex auto-detects schema-index changes.
...
R3 cursor continuation deferred to 0.7.2. Spec at `docs/specs/0.7.1-agent-first-aggregate.md`. Six new error codes: `RESPONSE_TOO_LARGE`, `CURSOR_INVALID`, `SCHEMA_REF_CHAIN_INVALID`, `FILTER_BETWEEN_INVALID`, `FILTER_EMPTY_ARRAY`, `FILTER_OP_INVALID`. 689 tests passing (+49 over 0.7.0 baseline), no new dependencies.
...
- **0.12.0 (next minor)** — folds in HTTP `/mcp` Origin validation (MCP Streamable HTTP conformance, DNS-rebinding defense; merged unreleased), generated agent-instruction correctness fixes (merged unreleased), the managed-instruction lifecycle (merged unreleased: stamped MAAD.md/_skills files, `maad instructions check/refresh` + `maad_instructions` MCP tool, static MAAD.md — **behavior change: reindex no longer rewrites MAAD.md**; boot emits an `instructions_outdated` advisory only), and structural field constraints (merged unreleased: `max_length`/`soft_max_length`/`multiline` on string fields, code-point lengths, `includeConstraints` preflight — **behavior change: unknown field-definition keys now fail schema activation with `SCHEMA_INVALID`**; audit deployed schemas for stray keys before rolling) alongside the concurrency/durability work.
