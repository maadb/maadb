...
Raw committed and working bytes are each limited to 256 KiB. Canonical content has depth, node, and expansion bounds. The complete MCP tool response is limited to the smaller of 1 MiB and the existing configured response cap (64 KiB by default). Responses are rejected, never truncated or projected down.
