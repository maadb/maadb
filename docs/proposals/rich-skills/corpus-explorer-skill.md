...
- Sparse realized graph, rich mention/prose links → load the
  graph/ontology skill and run its densification recipe against the map's
  "mention overflow" column.
...
- No visualization: the map is markdown tables. Rendering it as an
  interactive graph or dashboard is host-side work (see the pack's
  `host-boundary.md`).
...
1. Stage 4's edge-usage measurement (`maad_aggregate` per ref field) is the
   protocol's only per-edge loop. On schemas with many ref fields this is
   several calls; fine as-is, or is that the first candidate for Tier 2
   usage-data collection ("edge census" composite)?
2. The corpus-map template proposes a de-facto standard filename
   (`_skills/local/corpus-map.md`). Should managed skills name overlay files
   this concretely, or only describe the content and let projects choose
   names? (Concrete names help agent-to-agent continuity; they also
   colonize the project's namespace.)
3. Is `maad_scan` worth a stage for projects with un-imported sibling
   markdown (files outside the registry's typed paths), or does that belong
   solely to `_skills/import-guide.md`?
