...
export function register(server: McpServer, ctx: InstanceCtx): number {
  ...
  server.registerTool('maad_query', {
    ...
    inputSchema: z.object({
      ...
      limit: z.number().int().min(1).optional().describe('Max results (default 50, capped at 500). Over-requests are clamped silently with _meta.limit_clamped set.'),
      ...
      depthMaxResults: z.number().int().min(1).optional().describe('When depth=cold|full, hydrate at most N rows (default 50, capped at 100). Beyond cap, remaining rows return at hot depth.'),
  ...
  server.registerTool('maad_search', {
    ...
    inputSchema: z.object({
      ...
      limit: z.number().int().min(1).optional().describe('Max results (default 50)'),
      offset: z.number().int().min(0).optional().describe('Skip first N results'),
      project: z.string().optional().describe('Project name (multi-project mode only)'),
    }),
  ...
  server.registerTool('maad_aggregate', {
    ...
    inputSchema: z.object({
      ...
      limit: z.number().int().min(1).optional().describe('Max groups to return (default 50, capped at 2000). Over-requests are clamped silently with _meta.limit_clamped set.'),
      project: z.string().optional().describe('Project name (multi-project mode only)'),
    }),
